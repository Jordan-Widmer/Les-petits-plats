...
function displayRecipes(recipeList) {
	...
	recipeList.forEach((recipe) => {
		...
		populateFiltersAndMap(recipe.ingrédients, recipe.ustensils, recipe.appliance, recipe)
	})
	regenerateFilters()
}
...
console.error("prout", currentlyDisplayedRecipes)

console.log("searchItems", searchItems(data.recipes))
const handleSearch = () => {}
...
function displayFilteredRecipes() {
	...
	const filteredData = flattedList.filter(
		(a, index) =>
			...
			flattedList.reduce((acc, b) => +(a === b) + acc, 0) === activeFilterList.length
	)
...
function populateFiltersAndMap(ingrédients, recipe) {
	if (Array.isArray(ingrédients)) {
		...
		filtersList.ingrédients.includes(ingrédients) ? null : filtersList.ingrédients.push(ingrédients)
		mappedList.ingrédients[ingrédient.ingrédient]
			? mappedList.ingrédients[ingrédient.ingrédient].push(recipe)
			: (mappedList.ingrédients[ingrédient.ingrédient] = [recipe])
	}
...
function handleFilterBtn(category) {
	...
	filtersList[category].forEach((filter, x) => {
		if (
			filter
				.toLowerCase()
				.includes(document.querySelector("." + category + "-input").value.toLowerCase()) ||
